...
def assign(onsets: list[OnsetEvent], diff: Difficulty, seed: int = 0, bpm: float = 0.0):
    ...
    always reproduces the same chart. `bpm` lets the jump pass measure note spacing
    in beats (for the clean-exit / jump-run rules)."""
    if not onsets:
        return []
